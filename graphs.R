...
png(file=paste(folder,"wc_general.png",sep = "/"), width=500, height=500, res = 100)
...
# happy
v <- sort(colSums(mtrain[which(tweets$class == "happy"),]), decreasing = TRUE)
d <- data.frame(word = names(v), freq = v)
head(d, 10)

png(file=paste(folder,"wc_happy.png",sep = "/"), width=500, height=500)
wordcloud(words = d$word, freq = d$freq, min.freq = 1,
          max.words=150, random.order=FALSE, rot.per=0.35, 
...
# sad
v <- sort(colSums(mtrain[which(tweets$class == "sad"),]), decreasing = TRUE)
d <- data.frame(word = names(v), freq = v)
head(d, 10)

png(file=paste(folder,"wc_sad.png",sep = "/"), width=500, height=500)
wordcloud(words = d$word, freq = d$freq, min.freq = 1,
          max.words=150, random.order=FALSE, rot.per=0.35, 
...
ggplot(data=d[d$freq > 0,], aes(y=d$freq[d$freq > 0], x=1:length(d$freq[d$freq > 0]))) + 
  labs(x = "Num", y = "Freq", title = "Frecuencias de términos en #Rio2016") +
  geom_point() + 
  geom_line(color = "Red")
dev.off()
...
ggplot(uni, aes(x=factor(1), fill=class))+
  geom_bar(width = 1) +
  coord_polar("y") + 
  labs(x = "", y = "", title = "Balanceo de clases en #Rio2016") + 
...
ggplot(count, aes(x=Var1, y = Freq, fill=Var1)) +
  ...
  labs(x = "", y = "Count", title = "Balanceo de clases en #Rio2016", fill = "") +
